import type { ProjectInput } from './types'

export const sizeProjects: ProjectInput[] = [
  {
    id: 'weapp-vite-wevu',
    label: 'weapp-vite + wevu',
    appDir: 'apps/weapp-vite-wevu',
    outputDir: 'dist',
    runtimeFiles: ['weapp-vendors/wevu-templateRef.js', 'weapp-vendors/wevu-watch.js'],
  },
  {
    id: 'weapp-vite-wevu-performance',
    label: 'weapp-vite + wevu performance',
    appDir: 'apps/weapp-vite-wevu-performance',
    outputDir: 'dist',
    runtimeFiles: ['weapp-vendors/wevu-templateRef.js', 'weapp-vendors/wevu-watch.js'],
  },
  {
    id: 'weapp-vite-native',
    label: 'weapp-vite 原生',
    appDir: 'apps/weapp-vite-native',
    outputDir: 'dist',
    runtimeFiles: [],
  },
  {
    id: 'uni-app-vite-vue3',
    label: 'uni-app vite vue3',
    appDir: 'apps/uni-app-vite-vue3',
    outputDir: 'dist/build/mp-weixin',
    runtimeFiles: ['common/vendor.js'],
  },
  {
    id: 'uni-app-x',
    label: 'uni-app x',
    appDir: 'apps/uni-app-x',
    outputDir: 'dist/build/mp-weixin',
    runtimeFiles: ['common/vendor.js', 'common/uniView.wxs', 'uvue.wxss'],
  },
  {
    id: 'mpx',
    label: 'mpx',
    appDir: 'apps/mpx',
    outputDir: 'dist/wx',
    runtimeFiles: ['bundle.js'],
  },
  {
    id: 'taro-vue3',
    label: 'taro vue3',
    appDir: 'apps/taro-vue3',
    outputDir: 'dist',
    runtimeFiles: ['taro.js', 'common.js', 'base.wxml', 'utils.wxs', 'comp.js', 'comp.wxml', 'comp.json'],
  },
]
